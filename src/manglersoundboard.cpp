...
ManglerSoundboard::ManglerSoundboard(Glib::RefPtr<Gtk::Builder> builder) {/*{{{*/
    this->builder = builder;

    builder->get_widget("sbWindow", window);
    builder->get_widget("sbHide", menuitem);
    menuitem->signal_activate().connect(sigc::mem_fun(this, &ManglerSoundboard::hide_activate_cb));
...
ManglerSoundboard::~ManglerSoundboard() {/*{{{*/
//    reset(true);
//    delete filedialog;
}/*}}}*/

void
ManglerSoundboard::show(void) {/*{{{*/
    window->present();
}/*}}}*/
void
ManglerSoundboard::hide_activate_cb(void) {/*{{{*/
    window->hide();
}/*}}}*/
#if 0
void
