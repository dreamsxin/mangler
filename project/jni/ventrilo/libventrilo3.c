 ...
 *
 * $LastChangedDate: 2009-11-30 06:01:28 +0100 (Mon, 30 Nov 2009) $
 * $Revision: 407 $
...
#include <unistd.h>
#include <math.h>
#if defined(HAVE_GSM_H) || defined(ANDROID)
#include <gsm.h>
#else
#include <gsm/gsm.h>
#endif
 ...
 */
int
v3_login(const char *server, const char *username, const char *password, const char *phonetic) {/*{{{*/
    char *srvname = NULL;
    char *srvport = NULL;
    char *sep = NULL;
...
void
v3_send_chat_message(const char *message) {/*{{{*/
    v3_event ev;
...
void
v3_change_channel(uint16_t channel_id, const char *password) {/*{{{*/
    v3_event ev;
...
void
v3_set_text(const char *comment, const char *url, const char *integration_text, uint8_t silent) {/*{{{*/
    v3_event ev;
...
// Really dirty function to start a processing thread for android.
// This is temporary until we move stuff out of ManglerNetwork.cpp
#ifdef ANDROID
void get_process_message(void) {
	do {
		_v3_net_message *msg;
...
int start_processing(void) {
	pthread_t thread;
	return pthread_create(&thread, NULL, (void *(*)(void*))get_process_message, NULL);
}
#endif // ANDROID
