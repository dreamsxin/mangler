 ...
 *
 * $LastChangedDate: 2009-11-30 05:04:38 +0100 (Mon, 30 Nov 2009) $
 * $Revision: 406 $
 ...
 */
int         v3_login(const char *server, const char *username, const char *password, const char *phonetic);
void        v3_join_chat(void);
void        v3_leave_chat(void);
void        v3_send_chat_message(const char *message);
void        v3_logout(void);
void        v3_change_channel(uint16_t channel_id, const char *password);
...
void        v3_set_text(const char *comment, const char *url, const char *integration_text, uint8_t silent);
